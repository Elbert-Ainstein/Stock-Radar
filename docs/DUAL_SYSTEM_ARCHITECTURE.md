...
- **Phase modulator (bubble runway).** A trade-BROKEN (overvalued on fundamentals) is NOT automatically "not investable." A structurally-real name in EARLY/MID euphoria with runway left is investable as a *ride-with-exit*; only in LATE euphoria does BROKEN mean sit out. The `risk_adj_ev_ratio` → BROKEN clamp must be modulated by **cycle/euphoria phase** (the analyst-panel clock — accelerating/peaking/decelerating/trough — already computes it; wire it into conviction). Guardrails: phase is a re-checked *current-state* read, not a banked "1-2y" forecast (else it violates facts-over-prediction); "ride" REQUIRES a defined exit trigger (phase → late ⇒ trim), else it's bag-holding; applies ONLY to strategic-HIGH names (never to rationalize chasing strategic-LOW momentum — that stays the momentum trap).
