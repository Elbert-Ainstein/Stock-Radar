...
## [2026-08-07] algo: the zero-trade investigation — v2.2 → v3.1, and the strategy now says when it can first act

A backtest of the settled-basis ladder produced no trades, then the wrong dates, then 22,118 error lines. Every one of those had a different cause and each is now diagnosed by the strategy itself, in the Log, in the run where it happens. The whole round in one entry (the intermediate commits shipped without one — logged here rather than left unrecorded).

**Six defects, six fixes.** (1) *Editor rejections* — `round()` on this platform takes ONE argument and `min`/`max` type-check float and require two positional args; `algo/check_and_simulate.py` now lints every call against the manual's signatures (arity, keyword names, int-literal-where-float, and taint through `x = f() or 0`) so a rejection is caught here rather than in the editor. (2) *Only the class may exist at module level* — no imports, no constants, no `if __name__` guard; the harness moved permanently back out of the strategy file, and `lint_structure` fails the build on any non-`Strategy` top-level node. (3) *An empty declared trigger slot disables the dialog's Next button* — the strategy declares ONE trigger symbol and the operator multi-selects the basket onto it. (4) *An INDEX as trigger symbol* (`.IXIC`) evaluates perfectly and is never buyable — `SymbolType.INDEX`/`PLATE` now prints `[NOT TRADABLE]` once and stops. (5) *Warm-up escaped as an exception* and errored symbols deliberately do not consume the day, so every trigger retried: 22,118 `[error]` lines burying the run. Price reads and indicator reads are now separate guarded blocks. (6) *Cash contention* — several securities sized against the same untouched cash; a shared per-day committed budget fixes it, and the regression measures actual dollars from the trade log rather than trusting the counter the fix maintains (without the fix, 12 symbols order $30,740 against $20,000).

**The two that were not defects — and are the real finding.** A run set to start 2025-01-31 began 2026-06-08: the backtest window is the **intersection of every symbol's history**, and one basket member (`US.SPCX`) had no prices at all, clamping 18 months to 8 weeks. And with the window honoured, the first buy landed 2025-11-19 — 207 sessions after the Feb 2 start, against a `trend_period + 1 = 201` requirement. Neither is a bug; both are invisible unless the strategy says so. It now does: `[NO DATA] <sym> has NO price history in this window … can CLAMP THE WHOLE BACKTEST`, said once, distinct from `[warming up] <sym>: session N of ~201`.

**v3.1 (this commit)** closes the loop by answering the question before it is asked. The `[first trigger]` line no longer says "add ~201 sessions" and leave the operator to do calendar arithmetic — `_est_ready()` converts sessions to a date (5 per 7 days plus a holiday allowance) and the line reads *"Earliest possible entry, after ~201 sessions of warm-up: 2020-10-14. Anything before that date is arithmetic, not strategy."* `type(d).fromordinal` sidesteps the no-imports rule.

**Acceptance: `python3 algo/check_and_simulate.py` = 38 checks green (structure lint + call lint + 6 simulated scenarios + regressions), offline, no platform needed. Regressions pin each defect above by its symptom — including that the first-trigger line contains a concrete date ~201 sessions out, that warm-up is never logged as an error, and that a no-data symbol is reported once and separately.**
...
**1 · "More analysts, complex personalities and philosophies — but the same goal."** The panel was three seats hardcoded in three places. It is now a registry (`scripts/socratic_panel.py`): a seat is a row plus a prompt file. Five new analysts, each with a school, an evidence standard, and a **declared failure mode** it must self-report every run (`bias_check`) — a persona with no named bias is a caricature that argues its corner forever: **Supply-Chain Empiricist** (the truth is upstream; order books and customer capex lead revenue by 2-4 quarters), **Capital-Cycle Analyst** (high returns invite capacity; supply response kills more theses than demand disappointment), **Technologist** (physics decides; demonstrated vs announced; technical moat vs merely temporal), **Base-Rate Statistician** (outside view, management's own forecasting record, and an explicit outlier test), **The Owner** (the disciplined steelman).

**The structural finding that forced the last one:** the original trio was Fundamentals ("assume growth decays toward historical rates"), Regime, and Adversarial ("you are trying to break one") — two bearish priors and **no seat assigned to build the strongest honest bull case**, inside a system whose entire purpose is finding 10× names. That panel does not evaluate asymmetry, it filters it out. The Owner seat closes it, held to the Adversary's evidence standard: it must set `case_supportable: false` when the facts don't carry a bull case. Pinned by `test_panel_is_not_structurally_bearish`.

Convergence is enforced, not hoped: every seat receives `[PANEL_ROSTER]` (who else is in the room, don't duplicate their work) and is told that disagreement counts only when it is a **different answer to a checkable fact** — taste disagreement is discarded downstream. The synthesis now weights each verdict **against its own declared bias** (a bear call from a seat that declared a bear failure mode is weak evidence; the same call from the Owner is strong).

**2 · "Don't let random news get to you. Focus on the facts that matter."** New `config/watched_facts.yaml` + `scripts/watched_facts.py`: per ticker, the specific causal chain the thesis rides on and the categories that are **inadmissible as evidence**. MU is the operator's own example — HBM sold-out horizon, hyperscaler capex direction, datacenter mix, industry capacity additions, contract (not spot) pricing, qualification slips; noise: price-target changes, daily spot ticks, generic AI-bubble commentary, headlines about the stock rather than the business. Every seat is bound: a claim traces to a listed fact or to `proposed_facts`, or it is decoration. A fact that cannot be answered is **declared**, never guessed around. Absence is loud — a ticker with no spec renders a NO SPEC ON FILE block, never silence.

**1b · "Get inspiration from great investors."** Each seat now carries a `lineage` — the practitioners whose documented METHOD it applies, and, in the same breath, **what that school demonstrably cost them**. Fundamentals: Graham's margin of safety and Terry Smith's quality-at-a-price — and that Graham's own method systematically missed the great compounders. Regime: Soros's reflexivity, Druckenmiller on liquidity, Steinhardt's variant perception — and that reflexivity is unfalsifiable without a stated thesis. Adversary: Chanos's structural shorts and Schilit's forensic accounting — and that Chanos was right-and-early on Enron and wrong-and-expensive on Tesla, so objections must be dated. Supply-chain: Fisher's scuttlebutt, Lynch's know-what-you-own — and that scuttlebutt scales badly. Capital-cycle: Marathon/Chancellor, Marks on cycle position, Grantham on margin mean-reversion — and that the school is famously early. Technologist: Grove's strategic inflection points, Fisher on research organizations — and that Christensen's disruption framework predicted the iPhone would fail. Base rates: Mauboussin, Tetlock, Kahneman & Tversky — and that a reference class applied bluntly rejects every outlier at its cheapest. Owner: Buffett's five-year test, Munger's inversion, Nick Sleep's scale-economies-shared — and that long-horizon conviction is one step from stubbornness, which is why Sleep closed the fund rather than drift.

**Method, not mimicry** is the enforced design rule: no prompt says "you are X" or asks "what would X do" — impersonation yields pastiche, remembered quotes and borrowed authority standing in for analysis. The injected block ends with the test each analyst must apply to itself: *if your reasoning would survive with every name deleted, it is analysis; if it would collapse, it was costume work.* Pinned by `test_no_prompt_instructs_impersonation` and `test_lineage_block_forbids_impersonation`. All eight persona prompts bumped to v2 (this also repairs a bookkeeping slip: the legacy trio's content changed earlier in the day while their version strings stayed v1, so the cohort key could not see the edit).

**3 · "Watchman and messenger mechanic isn't 100% refined."** Three real gaps, all closed: **(a) consult lifecycle** — a fired ticket was invisible forever after; nothing tracked whether the question was ever answered, which for a system whose only action is asking is the failure that makes it decorative. `engine/consult_state.py` counts, ages and surfaces open questions on EVERY brief, marks them OVERDUE past 7 days, reads answered-ness from the document (signature line or ticked door), and never escalates into action — a louder line on a page, never a decision. **(b) Ladders** — a wire fired once and went silent; cross $1,325, trim, and nothing watched $1,500 again. `ladder: [1500, 1800]` re-arms at the next **pre-declared** rung; the machine climbs the operator's ladder and never invents a rung. **(c) Dated signposts** — kill triggers that are dates, not prices ("certification decision by Q2") had no home: the date passed silently and the thesis kept its conviction. A `{type: signpost, due: ...}` wire is never price-adjudicated; on its due date the machine opens a REVIEW consult that **asks** (CONFIRMED / NOT_YET — a slipping date is itself evidence / OBSOLETE), because it cannot observe whether an agency ruled.

**Cohort break (declared, once):** the seated roster is now part of the instrument's identity — `panel_version` joins the cohort key (`checkpoint_seal.JUDGMENT_PROMPT_KEYS`), so any roster change resets the calibration clock rather than silently mixing two measuring devices. `socratic_analyses` gains `panel` + `panel_version` (in `PENDING.sql`; the insert now strips-and-retries like the thesis writer, so nothing breaks pre-migration).
...
**The two criticals — both law-2 violations the flip tests didn't catch:**
- **UTC settlement boundary.** "Settled = dated before today" used the UTC date, so a Mon–Thu evening run (19:00–24:00 ET in winter) settled and ADJUDICATED the same-day US close — the exact INTC/MU failure the constitution memorializes. Fixed at three layers: `exchange_today(ticker)` computes the boundary in the ticker's exchange-local tz (`market_calendar.CALENDAR_TZ`); `append_rows` force-demotes any settled row dated ≥ exchange-today at the choke point (`demoted:same-day (law 2)`); `adjudicate` refuses even a CSV-resident same-day settled row (defense in depth). All three pinned by tests.
- **Conflict flag never blocked anything.** `latest_settled` filtered flagged rows OUT, so a disputed latest print silently slid adjudication back to a stale clean date — `blocked_on_conflict` was dead code. Now the last write per date is surfaced including flags; a conflicted latest print BLOCKS wires (verdict says why, surfaces the disputed date), blocks euphoria (logged), and lands valuation in declared gaps.
- **Also fixed (HIGH/MEDIUM):** machine state moved out of `config/tripwires.yaml` into `data/wire_state.yaml` (first fire was destroying all operator comments via yaml round-trip; config is human-only now, byte-identical after fires — tested); the armed seed flip-wires shipped in-the-money and would have opened placeholder consults on the first live run (now `status: retired`; tests arm their own copies); stooq failures returned bare `[]` — the two-source check could die silently for months (now every non-ok status is logged/printed, single-source rows counted); NaN closes passed the cross-check as "verified" and poisoned valuation (rejected at the choke point, flagged in cross_check, gap in valuation); None shares/cash valued at 0 with the book reporting complete (declared gaps); premarket gated the whole pass on XNYS only (per-ticker calendars; HK runs on US holidays); fetch universe now includes armed-wire tickers (a wire on a non-held name could never adjudicate); anti-parabola measured from the window LOW (a flat round-trip flagged +150% — now measures vs the window-start close per §IV valley doctrine, declares short history); premarket exits 1 on degraded runs; README cron line had the DST mapping inverted (now `CRON_TZ=America/New_York`); host `.gitignore` `out` rule swallowed `portfolio-machine/out/.gitkeep` (anchored to `/out`).
- **Tests 25 → 49** and de-seed-coupled: scenario suites run on test-owned config (`flip_machine`, `euphoria_machine`) so replacing `SEED_REPLACE` values — the instructed operation — breaks nothing; new coverage for law-2 choke points, conflicted-latest, calendar honesty (HK≠US holidays, unknown-suffix→None), valuation gaps, config integrity (clause→wire refs, law-1 action shape, catalyst validation), and the premarket entrypoint (offline smoke, fire-propagates-to-consult).
- **Synthesis:** `docs/decisions/RADAR_MACHINE_SYNTHESIS_2026-07-28.md` — how Radar (opinion factory) and the Machine (restraint machine) fit, the four real conflicts (the advise line, spot vs settled, two graders, two sizing authorities), the cortex/brainstem supremacy clause, and five proposed Phase-2 build items. Awaiting owner decision.
...
- **L7 — hypothesis intake (`data/hypotheses/` + `scripts/hypotheses.py`).** One markdown file per hypothesis (frontmatter: status/tickers/horizon; body: claim / mechanism / S1–S6 signatures / DATED kill conditions — `TEMPLATE.md` shipped, `.gitignore` allowlisted). Active hypotheses naming a ticker are injected into that ticker's thesis run at the existing `[MEMORY_SECTION]` slot with explicit **falsification framing** ("your job is falsification, not origination") — `thesis_v3.md` is NOT edited, an empty folder yields a byte-identical prompt, and a malformed file warns and skips (never fails a run). Cohort-safe: `run_thesis.py` is not a `JUDGMENT_FILES` member and no judgment prompt changes — verified against `checkpoint_seal.py` before building.
- **L2 slice 1 — `scripts/discovery_screens.py` (S1 + S3 → STALK).** Discovery and allocation are different machines: the screens are diagnostic and NOTHING here sizes a position. **S1** (the MU/EDGAR lesson as code): a revenue sanity-check firing inside a SUCCESSFUL fetch is a regime-change signal — and finance_data's fail-open branch is respected: an `EDGAR cross-check unavailable` warning (foreign filer / no CIK / network) means the anomaly was NEVER verified and S1 refuses to pass (review catch — the first cut would have claimed SEC survival for unverified prints on multi-market names). **S3**: 13F adds parsed from gen-2's REAL artifact — the comma-joined `source` tag string `discovery_13f.upsert_candidate` maintains (`13f_{manager}_{quarter}_{kind}`), NOT an imagined jsonb (review catch: the first cut's live path was dead on arrival) — with analyst coverage ≤ 2 via operator `--coverage` (no live coverage source is wired yet; unknown coverage never passes, and 13F-adds-without-coverage is stated, not silent). Output is a **STALK**: new `stance` + `trigger_price` fields on `discovery_universe` (migration `supabase/2026-07-08_discovery_stalk.sql`; persistence is UPDATE-first because an upsert's insert tuple would trip the table's NOT NULL market/source columns — review catch) — deliberately never a `conviction` value. Trigger from evidence, never invented: operator `--trigger` > thesis actionability price (`risk_adj_target ÷ scaled LOW bound` — the spot at which the trade gate stops saying BROKEN, on the thesis's own clock) > NULL-and-stated.
- **L8 honored structurally:** every STALK is sealed into `prediction_log` (`stalk-` run_id, NO `reasoning_fingerprint`) — `run_checkpoint.is_gradeable_seal` classifies it `not_socratic`, so the calibration loop cannot be poisoned; a future stalk-grader inherits a clean history. The evidence payload rides `context_inputs` — a column that both exists on the table and survives `prediction_logger._coerce_row`'s whitelist (review catch: a `notes` key was silently dropped client-side, which would have left every seal evidence-free). Pinned through the real coercion + grader filter in tests.
- **L5 alert leg:** `refresh_prices.py` now checks stored STALK triggers against the quotes it just fetched (fetching quotes for off-watchlist STALK names too), prints a loud breach line, and stamps `trigger_breached_at` once (pre-migration DBs skip gracefully).

**Acceptance (falsifiable):** (1) the design doc's own criterion — the SNDK-at-$38.50 shape passes S1+S3 into a STALK with a trigger while `enforce_trade_gate` on the same day's numbers still clamps to BROKEN/0% (`test_discovery_screens.py::test_sndk_shape_...`); (2) a STALK seal row survives `_coerce_row` WITH its evidence and is `not_socratic` to the REAL grader filter (pinned imports, not copies); (3) live dry-run: `discovery_screens.py SNDK --dry-run` fires S1 off the genuine supercycle print and derives trigger $1154.74 from the latest thesis (risk_adj_target/0.95) at spot $1617.70; (4) EDGAR-unavailable anomalies never pass S1; the gen-2 tag string `13f_druckenmiller_2026Q1_new_position` parses to a manager/kind add; (5) empty hypotheses folder → byte-identical thesis prompt; bracket tokens in hypothesis bodies are neutralized (the fill() splice class); comma-separated `tickers:` without YAML brackets still match. Reviewed by a 2-lens adversarial pass: 7 confirmed findings (3 major integration defects the unit tests had masked) all fixed before commit. Suite **281 passed** (+27; only the two known stale LITE fixtures deselected). **STALK persistence is gated on applying `supabase/2026-07-08_discovery_stalk.sql`** (the seal and screens work without it).

## [2026-07-08] Lesson L4 shipped — no invisible opinions — plus hygiene batch (grader cron, watchlist refresher, two latent bugs)

L4 build-table item 2 (`docs/design/HORIZON_DISCOVERY_TYPEA_2026-07-02.md`), informed by the 2026-07-08 backtest campaign on `experimental/ai-backtester` (which prototyped the parameter block and proved the diagnosis distinction: deep sub-0.95 ratios are target-bound, band-edge ratios are clock-bound).

- **`scripts/run_parameters.py` (NEW).** (1) `build_thesis_run_parameters()` — one JSON blob per thesis run stating every parameter the verdict was judged under: model/prompt/temperature/max_tokens, spot + source (live vs EOD provider), horizon clock (config vs used vs out-of-bounds-fallback, with bounds), the **post-scaling clamp table**, archetype + dcf_role (config-pinned or default routing), allowlist size, IR-domain presence, memory size. (2) **Zero-result gate diagnosis**: `diagnose_verdict()` states which gate killed a non-actionable name and *what would have to change* — the ratio (and implied risk-adj target price at the run's spot) each band needs, the horizon clock that would flip it to LOW-actionable via `t^(h/1.25)` algebra (or "no in-bounds horizon fixes this — the market is expensive, the ruler is not the problem"), and whether a kill-gate relax route exists for its archetype. `diagnose_sweep()` + CLI (`python scripts/run_parameters.py --diagnose`, pre-migration select fallback) answer the 6/6-BROKEN question in one run. Distinguishes gate-clamped from model-emitted refusals.
- **`scripts/run_thesis.py` (money-path, minimal + guarded).** After the gates: builds the block, logs `[params] {...}`, prints the per-name `[gate_diagnosis]` whenever the verdict is 0%-position, and persists `run_parameters` on the theses row. Entire block in try/except — a params failure can NEVER fail a run (logging must not break the money path); verdicts/gates are untouched (block runs after both gates, reads only). Also fixes the **IR-cache precedence bug** (`a and b and c or d` bound as `(a and b and c) or d` — an empty company name made `"" in domain` true and cached the first cited domain as the IR site; both branches now require a company name). New migration: `supabase/2026-07-08_theses_run_parameters.sql` (one jsonb column + verification query; strip-and-retry safe until applied).
- **`write_to_supabase` strip budget made DYNAMIC (`len(row)+1` attempts, was `range(5)`).** The review caught the prediction_logger off-by-one recurring: the new column makes SIX theses columns potentially missing on a fully pre-migration DB, which would have exhausted the fixed budget and lost the ENTIRE verdict row. Simulation-tested at 6 and 8 missing columns (`test_thesis_strip_budget.py`); non-column errors still raise.
- **`.github/workflows/checkpoint-grader.yml` (NEW).** CLAUDE.md has called `run_checkpoint.py` "the cron grader" since June but NO schedule ran it anywhere (the host crontab targets a different machine) — T+30 grades were ripening ungraded. Now: daily 07:30 UTC + manual dispatch with a dry-run input. Grader is source-filtered and idempotent on already-graded rows.
- **`scripts/refresh_prices.py`.** `get_watchlist_tickers()` read the DORMANT `discovery_universe` (status='watchlisted', stale since 2026-05) instead of the `stocks` table, and the watchlist.json fallback only fired on exception, never on empty — the scheduled 5-minute refresh was polling the wrong universe. Now: `stocks` where active, falling back to watchlist.json on error OR empty.
- **`scripts/event_reasoner.py`.** Missing `import sys` — the log-only calibration path (gap (a), 2026-07-02) uses `file=sys.stderr` and would NameError the moment any calibration ratio ≠ 1.0 existed.

**Acceptance (falsifiable):** (1) a thesis run's log carries `[params]` with the post-scaling clamp table and `[gate_diagnosis]` on any 0%-position verdict (wiring); (2) `horizon_to_reach(0.90, 0.95)` ≈ 2.57y and `horizon_to_reach(0.5681, 0.95)` = None — the campaign's two regimes, pinned; and `horizon_to_reach(0.96, 0.95)` ≈ 0.99y, NOT "already clears" (sub-default clocks raise the bar); (3) diagnosis thresholds are derived from `trade_gate.CLAMP_TABLE` **by band name** (a Step-12 retune cannot leave the solver on a stale bar); (4) the six-pending-columns pre-migration insert persists the row minus the stripped columns (`test_thesis_strip_budget.py`); (5) `refresh_prices` returns the active stocks-table tickers and falls back on empty. Reviewed by a 3-lens adversarial pass (money-path safety / diagnosis math / ops): 13 confirmed findings (2 major — the strip-budget exhaustion and the unscaled-threshold early return) all fixed before commit; the sweep CLI gained staged pre-migration select fallback + active-watchlist filtering (stale retired-ticker rows would suppress the zero-result alarm), the fallback flag is derived independently of the enforcement record, string-typed ratios are coerced like the gate coerces them, and the kill-gate messages are honest that pre-revenue routing isn't knowable from a persisted row. Suite **254 passed** (`test_run_parameters.py` 26 + `test_thesis_strip_budget.py` 4 new); the only failures are the two pre-existing stale LITE engine fixtures which now RUN (live keys present in `.env`) instead of skipping — reproduced identically without this diff; re-capture remains the operator TODO. **Live persistence of `run_parameters` is gated on applying the 2026-07-08 migration** (alongside the two still-pending 2026-07-02 files).

## [2026-07-07] ROADMAP item 2 shipped — the structural axis is visible everywhere a verdict renders

Completes dual-system Step 1's original acceptance criterion (persisted 2026-06-27; surfaced now): a verdict reads **"strategic HIGH / trade BROKEN / buy below $X"** instead of a bare BROKEN. Frontend + one API route only — no money-path files touched. `tsc --noEmit` clean; Python suite untouched (no `.py` modified).

- **Dashboard watchlist (`app/dashboard/StockRow.tsx`, `Dashboard.tsx`).** The CONVICTION cell renders a dual pill — `S·HIGH` (Type A, price-independent) + `T·BRKN` (Type B, gate-clamped) — with the full sentence (strategic / trade / buy-below / EV ratio / clock / kill-gate routing) in the cell tooltip, and a `†` marker when the kill gate relaxed the verdict (also on the rare `kill_gate_override`-without-`strategic_conviction` row — the gate treats a missing strategic verdict as pass). Rows with no structural data render the bare trade pill byte-identical to before (no wrapper, no tooltip). Grid column widened 96→132px (`SR_GRID`), header min-width 1208→1244.
- **Stock page thesis panel (`app/dashboard/ThesisHeader.tsx`).** `DualConvictionBadges` (labeled strategic + trade) in the panel and inline card; new footer chips for `risk_adj_ev_ratio` ("recomputed by the trade gate") and `thesis_horizon_years` ("Clock 3y" — L1: every verdict states which clock it was judged on); kill-gate routing line (`BROKEN → LOW — reason`) and Model D vision-vs-floor line (like-for-like ×, "additive, not a verdict") when persisted. `ConvictionBadge` now uppercases before the style lookup (matching every other surface and `kill_gate.py`'s server-side normalization); `flex-wrap` is applied only when structural content is present so legacy rows keep the pre-change layout at every viewport width.
- **Data loaders (`lib/data.ts`).** New `ThesisRun` fields + `KillGateOverride`/`ModelDBracket` interfaces mirroring `kill_gate.py`/`model_d.py` shapes. One shared `selectThesesWithStructural()` selects the structural columns optimistically with a **staged retry** — PostgREST rejects the whole select if any column is missing, and the columns ship in TWO separate migration files, so it tries (1) all five (`strategic_conviction, risk_adj_ev_ratio, kill_gate_override, model_d_bracket` + `thesis_horizon_years`), (2) without `thesis_horizon_years` (horizon migration unapplied), (3) legacy columns only (consolidated migration unapplied), warning with the failing stage each step. A partially migrated DB therefore still renders the four consolidated columns instead of silently disabling the whole feature. Used by `loadStocks` (dashboard) and `loadStocksForModel` (model page, previously trade-only columns).
- **Ask AI fix (`app/api/ask/route.ts`).** The 2026-07-02 Option A repoint selected structural columns with NO fallback — on the pre-migration live DB the whole theses query errored and every stock's VERDICT OF RECORD read "no thesis run recorded" (live bug since that commit). Now uses the shared fallback helper; verdict line gains `clock=Xy` when persisted.
- **Model detail page (`app/model/[ticker]/detailed/`).** `ThesisData` typed with the structural fields (`/api/thesis/[ticker]` already `select("*")`s them through); headline strip conviction pill becomes `S·HIGH / T·BROKEN · 10%` with explanatory tooltip; ThesisTab conviction tile relabels "Conviction (trade)" and adds a colored `strategic X` sub-line. Legacy rows unchanged on both surfaces.

**Acceptance (falsifiable):** (1) a theses row with `strategic_conviction=HIGH, conviction=BROKEN, buy_below=436` renders S·HIGH + T·BRKN pills on the watchlist with tooltip "strategic HIGH / trade BROKEN / buy below $436…"; (2) a legacy row (no structural columns) renders the single trade pill exactly as pre-change on all three surfaces — no wrapper span, no tooltip, no layout-class change; (3) against a pre-migration DB, dashboard, model page, and Ask AI all still show thesis rows (fallback fires, warn logged) — Ask AI previously showed none; (4) against a partially migrated DB (consolidated applied, horizon not), the four consolidated columns still render and the warn names the horizon migration. Diff reviewed by a 3-lens adversarial pass (correctness / legacy-regression / consistency); all five confirmed findings fixed before commit. **Live rendering of real values remains gated on the operator applying BOTH pending migrations** (`supabase/2026-07-02_consolidated_pending.sql` AND `2026-07-02_theses_horizon.sql` — the horizon column is not in the consolidated file).
...
**The two that mattered (both in my own L1 implementation):**
- **The model could choose its own gate clock (CONFIRMED HIGH).** `enforce_trade_gate` read `thesis_horizon_years` from the LLM's closing JSON AHEAD of the operator config — the hard gate took timing instructions from the thing it exists to constrain (a model emitting `10` alongside a 30%-below-spot target turned BROKEN/0% into LOW/10%). Fixed: the config/default is always passed explicitly and always wins; a conflicting model-emitted horizon is ignored and flagged (`horizon_source`, `model_horizon_ignored`, "[MODEL-EMITTED horizon IGNORED]" in the log).
- **Horizon scaling was loosening the gate against a mismatched number (CONFIRMED HIGH).** thesis_v3.md still pins `risk_adj_target` to a 12–18-month date, so scaling the BROKEN edge down for a 3y config entry let a 15-month EV 8% below spot escape the kill band. Fixed: **conservative-only scaling** — thresholds are floored at native values (a configured clock only tightens) until the prompt carries the horizon (L3 batch). This retracts the earlier "0.90×@3y = LOW" note: it stays BROKEN for now, by design. Model D's bracket clock correctly stays 1.25y for the same reason (documented at the call site).

**Also confirmed and fixed:** `event_reasoner` used `sys.stderr` without importing `sys` — a NameError would have discarded ALL reasoned events whenever calibration ratios existed (regression test exercises the exact path); L4's zero-result diagnosis was wired only into a thesis path production sweeps never execute — now also runs daily in the grader over the latest theses rows (pre-migration-tolerant select); `generate_model`'s seed-only guard failed OPEN on a Supabase read error (would overwrite operator kill text on a timeout — now fails closed with a warning); Ask AI ignored `thesesRes.error`, silently converting query failures into "no thesis run recorded" (now injects a distinct THESIS DATA UNAVAILABLE marker; the `get_portfolio_summary` refresh tool also carries the verdict line now); `PENDING.sql` omitted the same-day `thesis_horizon_years` migration (added — the completeness contract holds); `_load_thesis_horizon` swallowed config errors silently (now warns loudly on unreadable file/non-numeric/boolean values; accepts numeric strings); `gate_artifact_analysis` mislabeled not-gate-caused clamps as "upper band," excluded them from summary counts, and read "clamps look honest" on all-data-problem sweeps (all reworded/counted; `horizon_to_clear_low`'s ≥0.95 short-circuit removed); `--no-models` now reflected in the advertised stage list/progress totals; a fully failed mini-pipeline no longer exits 0 (analyst/thesis failures propagate); TS `killConditionEval.status` unions gained `"unknown"`.

**Chores automated out of existence (same directive):** the cron entrypoint self-updates (`git pull --ff-only`, guarded, logged, `STOCKRADAR_NO_PULL=1` opt-out) — "pull on the cron machine" is gone; `supabase/PENDING.sql` is the one rolling idempotent migration file, auto-applied on merge by `.github/workflows/apply-migrations.yml` once a `SUPABASE_DB_URL` secret exists (loud green no-op until then) — "run this SQL" is gone.

## [2026-07-02] Lessons L4/L5/L7 shipped — no invisible opinions, signpost-grade kills, hypothesis intake

Second slice of the lessons spec (owner: "build what is next"; L1 downside-edge behavior blessed). Suite **215 → 227 passing** (+12).

- **L4a Parameter-block emission (`run_thesis.py`).** Every thesis run now emits and persists `run_parameters` (jsonb; migration `supabase/2026-07-02_theses_run_parameters.sql`): the clock, the horizon-scaled clamp thresholds, archetype routing, spot, allowlist size, sampling params, and which gates acted (trade gate clamped / kill gate overrode / Model D ran). A verdict is now auditable against its own configuration — the engine has no invisible opinions.
- **L4b Zero-result auto-diagnosis (`trade_gate.py` + `run_pipeline.py`).** A thesis sweep whose verdicts are ALL non-actionable (nothing better than BROKEN) now auto-generates a **gate-artifact analysis** instead of a silent shrug: per name, the ratio's annualized read on its clock, and whether ANY clock within 10y would escape the BROKEN band (`horizon_to_clear_low`: 0.90 clears at ≥2.57y; 0.55 never — a tape call). Summary line answers the 6/6-BROKEN question directly: "ruler-suspect — review thesis_horizons.json" vs "tape call — consider discovery mode (STALK), not force-buying."
- **L5 Signpost-grade kill conditions (`kill_condition_eval.py` + `run_thesis.py`).** New `ARCHETYPE_KILL_TEMPLATES` (per-archetype signpost classes: prints/guidance for garp, capex-ASP-inventory for cyclicals, NRR/renewals for compounders, dated regulatory/deployment/financing events for transformational, the event's own dates for special situations) appended to the evaluator's guidance; new advisory `lint_kill_triggers` flags vibe phrases ("competition intensifies", "sentiment…") and number/date-free triggers on every thesis run (`[kill_lint]` log). Prompt-side enforcement deferred to the L3 batch (single cohort reset).
- **L7 Human-hypothesis intake (`data/hypotheses/`).** `TEMPLATE.md` (claim / mechanism / signatures S1–S6 / dated kill conditions / horizon / status); `run_thesis` injects `data/hypotheses/<TICKER>.md` into the PRIOR CONTEXT block with falsify-don't-flatter instructions — no prompt-file change, no cohort churn. The human is the hypothesis generator; the engine is the falsifier.
- **Hardening:** `write_to_supabase`'s strip budget is now dynamic (`len(row)+1`, same fix as prediction_logger — it sat at exact capacity and `run_parameters` would have tipped it into failing whole inserts pre-migration); `.gitignore` whitelists `data/hypotheses/` and `data/memory/` (the latter matching the owner's backup decision).
...
- **Archetypes (4.1, approved as proposed; "more data the better" → Model D stays archetype-gated, so it runs on LITE/RKLB/ACHR).** All 6 watchlist names tagged in `config/ticker_archetype_overrides.json` (SNDK cyclical · RKLB/ACHR transformational · PLTR compounder · CELH garp). `kill_condition_eval` now resolves the archetype from the override config when callers pass none — before this, the cyclical/transformational guidance NEVER fired (analyst.py doesn't pass archetype) and every stock was kill-evaluated under GARP. **Acceptance met:** SNDK kill evaluation runs under CYCLICAL guidance (`test_kill_eval_routing.py`, 8 tests).
- **Option A — theses are the verdict of record (4.2).** (1) `generate_model` now only SEEDS `thesis`/`kill_condition` on rows that have none — the twice-daily regeneration no longer overwrites operator-edited text. (2) New `--no-models` flag on `run_pipeline`; `analyst-rebuild.yml` passes it — **the 2×-daily Opus scenario-model regeneration is retired** (models regenerate on demand: new ticker / operator). (3) **Ask AI repointed:** `/api/ask` now loads the latest thesis per ticker (conviction, strategic_conviction, targets, ratio, buy_below/trim_above, kill-gate override) as the "VERDICT OF RECORD" line with explicit instructions that it is authoritative for buy/sell and that legacy scenarios are display-only; also fixes the hardcoded retired 2025 Opus snapshot → `claude-opus-4-8`, and corrects the false "kill status is on the Dashboard card" guidance. (4) EventImpactsPanel relabeled "Signal blend (display-only)".
- **Gap (a) — event-magnitude calibration ratios LOG-ONLY** (`event_reasoner.py`). Decision logic: the math attributes each stock's total move to every pending event and pseudo-replicates daily rows, so one 30% pop can triple every event weight (clamped 3×) — invalid, and it scaled the live legacy blend twice daily. Ratios are still recorded per-event for observability; application requires explicit `CALIBRATION_APPLY=1` after the attribution math is rebuilt.
- **Gap (b) — kill evaluation fails CLOSED** (`kill_condition_eval.py`). Missing API key, request failure, or garbage model status now return `status='unknown'` (never `'safe'`); the existing UI renders no badge and un-suppresses the failure reasoning for non-safe statuses. "No kill condition defined" legitimately stays `safe`.
- **Gap (c) — `--auto-thesis-after` fixed and de-duplicated** (`run_pipeline.py`). The spawn crashed on undefined `HERE`/`REPO_ROOT` *after* the mini-pipeline (killing progress cleanup) AND was a redundant $3–5 double-run — `run_single_ticker` already runs the thesis inline (Stage 4). The flag is now an accepted no-op with a log line; `_spawn_auto_thesis` itself is fixed for future callers. Also fixes a Phase-3 regression: the single-ticker early return now returns True (was None → exit 1 on success).
- **Gap (d) — `fill()` single-pass substitution** (`run_socratic.py`). The sequential `str.replace` loop re-scanned inserted values, so a context block containing another block's token (macro block carrying a literal `[VIX]`/`[CHAIN]` line) got other blocks spliced into it mid-instruction — every judgment prompt carried context 2–3×. Now one regex pass over the original body only. Timing logic: this batch already modifies `target_engine.py`/`kill_condition_eval.py`, which are in `JUDGMENT_FILES` — the cohort key resets with this merge regardless, so this was the free moment; expect `version_cohort_break=true` on each ticker's next seal.
- **Gap (e) — Model D bracket made like-for-like** (`model_d.py`). The vision ceiling is a t=0 PV while the engine floor is an undiscounted 12–18-mo price; ratio-ing them raw deflated the vision lens by ~the 5-year discount factor. `bracket()` now discounts the floor to PV over `floor_horizon_years=1.25` at the same rate and reports the like-for-like ratio as the headline (`vision_over_floor_x`), keeping `vision_over_floor_x_raw` + `engine_floor_pv` for audit. Chosen basis: compare at t=0 (both numbers already live there once the floor is discounted); the 2026-06-26 LITE "0.8× inverted bracket" datum was substantially this artifact (~1.4× like-for-like). run_thesis log line shows both ratios.
