...
3. Check `config/watchlist.json` for the active stock list
4. If making changes to valuation logic, run `verify_model.py` after
5. If making changes to the pipeline, test with `--ticker <single>` before `--all`
6. If Supabase errors appear about missing columns, add them via SQL Editor — don't assume migration.sql is in sync
